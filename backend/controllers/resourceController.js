...
export const updateResource = async (req, res) => {
  try {
    ...
    const { quantity, name, status, availability } = req.body;
...
export const getResourcesByAgency = async (req, res) => {
  try {
    const { agencyId } = req.params;

    // Convert agencyId to ObjectId
    const objectIdAgencyId = new mongoose.Types.ObjectId(agencyId);

    const resources = await Resource.find({ ownerAgency: objectIdAgencyId });
    ...
    console.error("Error fetching resources by agency:", error);
    res.status(500).json({
      success: false,
...
//     const resources = await Resource.find({ ownerAgency: agencyId });
