...
export const registerAgency = async (req, res) => {
  try {
    ...
    console.log("name:" + name, "\nemail:" + email, "\npass : " + password, "\nAddress : " + JSON.stringify(contact), "\nphone: " + phoneNumber, "\nexpertise" + expertise)
    if (
      !name ||
      !email ||
      !password ||
    ...
    if (!process.env.MAPBOX_API_KEY) {
      console.error("MAPBOX_API_KEY is missing in environment variables");
    }
    ...
    try {
      ...
      console.log(geocodingResponse.data); // Handle API response
    } catch (error) {
